...
for(var x = 0; x < total; x++){
	...
	if (campo.type === 'text' || campo.type === 'password'|| campo.type === 'ticket-number'){
		...
		if(campo.name == "TicketNumber"){
			campo.value = "ALO10"
			campo.fileEvent = ("onchange")
			document.getElementById ("ticket-submit-button").fireEvent("onclick")
		}
		else{
			for(var i = 0; i < totalCampo; i++){
				...
				if (arrCampo[i].value == false || arrCampo[i].value == arrCampo[i].defaultValue){
					...
					if(arrCampo[i].type === "tel" || arrCampo[i].type === "ticket-number" ) {
						var str = 'ALO10';
